...
export function CalendarMiniMonth({
  ...
  return (
    <article className="shrink-0 rounded-xl border border-slate-200/90 bg-white p-2 shadow-sm">
      ...
      <section className="mt-1 grid grid-cols-7 gap-px text-center text-[8px] font-semibold text-slate-400">
        {["L", "M", "M", "J", "V", "S", "D"].map((l, i) => (
          <span key={`${l}-${i}`}>{l}</span>
        ))}
      </section>
      <section className="mt-px grid grid-cols-7 gap-px">
        {cells.map((d) => {
          ...
          return (
            <button
              ...
              className={`relative flex h-6 w-full items-center justify-center rounded text-[9px] font-medium tabular-nums ${
            ...
            >
              ...
              {hasEvent && !selectedDay ? (
                <span className="absolute bottom-0 h-0.5 w-0.5 rounded-full bg-sky-500" />
              ) : null}
            </button>
          );
        })}
