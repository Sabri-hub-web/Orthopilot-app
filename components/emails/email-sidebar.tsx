...
interface EmailSidebarProps {
  ...
  canGoPrev: boolean;
  canGoNext: boolean;
  onFilterChange: (tab: EmailFilterTab) => void;
...
export function EmailSidebar({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          {EMAIL_FILTER_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`inline-flex h-10 shrink-0 items-center gap-1.5 rounded-xl px-3 text-sm font-medium transition-all duration-200 ${
                  active
                    ? "bg-gradient-to-r from-[#6D28D9] to-[#7C3AED] text-white shadow-sm shadow-violet-500/25"
                    : "bg-slate-50 text-[#475569] hover:bg-slate-100"
                }`}
              >
                {tab.label}
                <span
        ...
        <div className="flex items-center gap-2">
          <div className="relative min-w-0 flex-1">
            ...
            <input
              ...
              className="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 pl-8 pr-2 text-xs text-slate-800 outline-none transition focus:border-violet-300 focus:ring-2 focus:ring-violet-500/15"
            />
          </div>
          <button
            type="button"
          ...
          <button
            ...
            title="Nouvel email"
            onClick={onCompose}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-[#6D28D9] to-[#7C3AED] text-white shadow-sm transition hover:opacity-90"
          >
      ...
      <footer className="flex shrink-0 items-center justify-between border-t border-slate-100 px-4 py-2">
        ...
        <div className="flex gap-1">
          <button
            ...
            onClick={onPrevPage}
            disabled={!canGoPrev}
            className="rounded-lg border border-slate-200 px-2 py-1 text-[10px] text-slate-600 disabled:opacity-40"
          >
            Préc.
          </button>
          <button
